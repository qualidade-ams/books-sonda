...
│   │   │   │   └── RequerimentosTable.tsx				   # Componente de tabela responsiva para exibição de requerimentos com layout de tabela HTML tradicional, checkbox de seleção geral no cabeçalho, colunas responsivas que se ocultam/mostram baseado no breakpoint (md/lg/xl/2xl), cabeçalho "Período de Cobrança" para melhor clareza da informação, informações condensadas em telas menores com dados agrupados na coluna principal, formatação de horas com horasUtils, cálculo de valores totais para tipos de cobrança específicos, badges coloridos por tipo de cobrança e módulo/linguagem, coluna de ações condicional controlada pela prop showActions com botões compactos (editar/excluir/enviar faturamento), modal de confirmação para envio ao faturamento, integração com sistema de permissões, acessibilidade completa e estados de loading/erro
