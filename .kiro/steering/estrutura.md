...
│   │   │   │   └── RequerimentosTable.tsx				   # Componente de tabela responsiva para exibição de requerimentos com layout de tabela HTML tradicional, checkbox de seleção geral no cabeçalho, colunas responsivas que se ocultam/mostram baseado no breakpoint (md/lg/xl/2xl), cabeçalho "Período de Cobrança" para melhor clareza da informação, informações condensadas em telas menores com dados agrupados na coluna principal, formatação de horas com horasUtils, cálculo de valores totais para tipos de cobrança específicos, badges coloridos por tipo de cobrança e módulo/linguagem, botões de ação compactos (editar/excluir/enviar faturamento), modal de confirmação para envio ao faturamento, integração com sistema de permissões, acessibilidade completa e estados de loading/erro
...
│   │   │   ├── LancarRequerimentos.tsx 				   # Página completa para lançamento de requerimentos de especificações funcionais (formulário de criação/edição, filtros avançados por módulo/linguagem/tipo de cobrança/mês/datas com tratamento especial para valores "todos" e layout simplificado em grid responsivo md:grid-cols-4, seleção múltipla com ações em lote, estatísticas em tempo real com layout de grid fixo, envio para faturamento individual e em lote, modal de reprovação com campos específicos para requerimentos reprovados incluindo controle de horas reprovadas e criação automática de requerimento de Banco de Horas, integração com sistema de permissões, interface responsiva utilizando componente RequerimentosTable para exibição tabular profissional com funcionalidades completas de seleção, edição, exclusão e envio para faturamento)
