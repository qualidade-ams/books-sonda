...
exports.sincronizarApontamentosIncremental = sincronizarApontamentosIncremental;
 ...
 */
async function buscarRegistroExistente(idExterno) {
    const { data, error } = await supabase
        .from('apontamentos_aranda')
        .select('data_ult_modificacao_geral')
 ...
 */
function prepararDadosApontamento(registro) {
    return {
        origem: 'sql_server',
 ...
 */
async function inserirRegistro(idExterno, dados) {
 ...
 */
async function atualizarRegistro(idExterno, dados) {
    const { error } = await supabase
        .from('apontamentos_aranda')
        .update(dados)
        .eq('id_externo', idExterno);
 ...
 */
async function sincronizarApontamentosIncremental(pool) {
    const resultado = {
        sucesso: false,
        total_processados: 0,
        inseridos: 0,
