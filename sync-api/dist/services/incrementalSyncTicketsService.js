...
exports.sincronizarTicketsIncremental = sincronizarTicketsIncremental;
 ...
 */
async function buscarUltimaDataSincronizada() {
 ...
 */
function calcularDataInicioComFolga(ultimaData) {
 ...
 */
async function buscarRegistrosModificados(pool, dataInicio, limite = 0 // 0 = sem limite, busca todos
) {
  ...
  `;
    const countResult = await pool.request()
        .input('dataInicio', mssql_1.default.DateTime, dataInicio)
        .query(queryCount);
  ...
  `;
    ...
    if (result.recordset.length > 0) {
        ...
        result.recordset.slice(0, 3).forEach((reg, idx) => {
        ...
        if (result.recordset.length > 3) {
            ...
            result.recordset.slice(-3).forEach((reg, idx) => {
 ...
 */
async function buscarRegistroExistente(nroSolicitacao, dataAbertura) {
 ...
 */
function deveAtualizar(dataSqlServer, dataSupabase) {
 ...
 */
function prepararDadosTicket(registro) {
    return {
 ...
 */
async function inserirRegistro(dados) {
 ...
 */
async function atualizarRegistro(nroSolicitacao, dataAbertura, dados) {
    const { error } = await supabase
        .from('apontamentos_tickets_aranda')
        .update(dados)
        .eq('nro_solicitacao', nroSolicitacao)
 ...
 */
async function processarRegistro(registro, index, total) {
    try {
        ...
        const { existe, dataModificacao } = await buscarRegistroExistente(registro.Nro_Solicitacao, registro.Data_Abertura);
        // Preparar dados
 ...
 * Busca TODOS os registros modificados (sem limite)
 */
async function sincronizarTicketsIncremental(pool) {
    const resultado = {
        sucesso: false,
        total_processados: 0,
        inseridos: 0,
