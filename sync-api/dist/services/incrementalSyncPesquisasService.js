...
exports.sincronizarPesquisasIncremental = sincronizarPesquisasIncremental;
 ...
 */
async function buscarRegistrosModificados(pool, dataInicio) {
  ...
  `;
    ...
    resultDistribuicao.recordset.forEach((row) => {
  ...
  `;
    ...
    if (result.recordset.length > 0) {
        ...
        result.recordset.slice(0, 5).forEach((reg, idx) => {
 ...
 */
function prepararDadosPesquisa(registro) {
    // Aplicar transformação automática para clientes com "-AMS"
 ...
 */
async function processarRegistro(registro, index, total, registrosIgnorados, camposAtualizados) {
    try {
 ...
 */
async function sincronizarPesquisasIncremental(pool) {
    ...
    try {
        ...
        const camposAtualizados = new Set();
