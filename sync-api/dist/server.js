...
const incrementalSyncApontamentosService_1 = require("./services/incrementalSyncApontamentosService");
const incrementalSyncTicketsService_1 = require("./services/incrementalSyncTicketsService");
const incrementalSyncPesquisasService_1 = require("./services/incrementalSyncPesquisasService");
 ...
 */
function formatarDataSemTimezone(date) {
    if (!date)
        return null;
    try {
 ...
 */
function gerarIdUnico(registro) {
    const partes = [
        registro.Empresa,
        registro.Cliente,
        registro.Nro_Caso
 ...
 */
function gerarIdUnicoEspecialista(registro) {
 ...
 */
function gerarIdUnicoApontamento(registro) {
    // Validar se os campos obrigatórios existem
 ...
 */
function gerarIdUnicoTicket(registro) {
    // Validar se os campos obrigatórios existem
...
app.post('/api/sync-pesquisas-incremental-test', async (req, res) => {
    ...
    try {
        ...
        console.log = (...args) => {
            const mensagem = args.map(arg => typeof arg === 'object' ? JSON.stringify(arg) : String(arg)).join(' ');
        ...
        const resultado = await (0, incrementalSyncPesquisasService_1.sincronizarPesquisasIncremental)(pool);
    ...
    try {
        ...
        while (continuar) {
            ...
            const resultado = await new Promise((resolve) => {
                ...
                sincronizarPesquisas(fakeReq, fakeRes, false);
            });
            // Acumular resultados
        ...
        resultadoGeral.mensagens.push(`Sincronização completa: ${resultadoGeral.total_lotes} lotes, ${resultadoGeral.total_novos} novos, ${resultadoGeral.total_atualizados} atualizados, ${resultadoGeral.total_erros} erros`);
        console.log('\n=== RESUMO FINAL ===');
 ...
 */
async function sincronizarPesquisas(req, res, sincronizacaoCompleta = false) {
    const resultado = {
        sucesso: false,
        total_processados: 0,
        novos: 0,
    ...
    try {
        ...
        let query;
        let ultimaDataSincronizacao = null;
      ...
      `;
        ...
        if (!sincronizacaoCompleta && ultimaDataSincronizacao) {
            request.input('ultimaData', mssql_1.default.DateTime, ultimaDataSincronizacao);
        }
 ...
 */
async function sincronizarEspecialistas(req, res) {
    const resultado = {
        sucesso: false,
        total_processados: 0,
        novos: 0,
    ...
    `;
        ...
        for (let i = 0; i < registros.length; i++) {
            ...
            try {
                ...
                const dadosEspecialista = {
                    ...
                    nivel: null,
                    observacoes: null,
                    status: (registro.user_active ? 'ativo' : 'inativo'),
                    autor_id: null,
                    autor_nome: 'SQL Server Sync'
                };
        ...
        resultado.sucesso = resultado.erros === 0;
        resultado.mensagens.push(`Sincronização concluída: ${resultado.novos} novos, ${resultado.atualizados} atualizados, ${resultado.removidos} removidos, ${resultado.erros} erros`);
...
app.get('/api/stats-pesquisas', async (req, res) => {
    ...
    `;
        ...
        if (grupo && grupo !== 'todos') {
            ...
            const pattern = grupoPatterns[grupo] || `%${grupo}%`;
    ...
    `;
        const request = pool.request();
        request.input('ano', mssql_1.default.Int, anoFiltro);
        if (grupo && grupo !== 'todos') {
            const grupoPatterns = {
                'FISCAL': '%FISCAL%',
                'COMEX': '%COMEX%',
            ...
            const pattern = grupoPatterns[grupo] || `%${grupo}%`;
            request.input('grupoPattern', mssql_1.default.VarChar, pattern);
        }
        const result = await request.query(query);
...
app.get('/api/debug-grupos', async (req, res) => {
    ...
    `;
        ...
        grupos.forEach(grupo => {
            ...
            const pertenceAosPrincipais = gruposPrincipais.some(principal => nomeGrupo.toUpperCase().includes(principal));
        ...
        const response = {
            ...
            grupos_detalhados: grupos.map(g => ({
                ...
                pertence_aos_principais: gruposPrincipais.some(principal => (g.Grupo || '').toUpperCase().includes(principal))
            }))
        };
...
app.post('/api/sync-apontamentos', async (req, res) => {
    try {
        ...
        // Executar sincronização incremental
        const resultado = await (0, incrementalSyncApontamentosService_1.sincronizarApontamentosIncremental)(pool);
        // Fechar conexão
        await pool.close();
...
app.post('/api/fix-null-fields-apontamentos', async (req, res) => {
    try {
        ...
        // Importar serviço
        const { corrigirCamposNull } = await Promise.resolve().then(() => __importStar(require('./services/fixNullFieldsService')));
        // Conectar ao SQL Server
        const pool = await mssql_1.default.connect(sqlConfig);
        console.log('✅ [API] Conectado ao SQL Server');
        // Executar correção
...
app.post('/api/sync-tickets', async (req, res) => {
    try {
        ...
        const resultado = await (0, incrementalSyncTicketsService_1.sincronizarTicketsIncremental)(pool);
        await pool.close();
        res.json(resultado);
    }
    catch (error) {
 ...
 */
async function sincronizarTickets(req, res, sincronizacaoCompleta = false) {
    const resultado = {
        sucesso: false,
        total_processados: 0,
        novos: 0,
    ...
    try {
      ...
      `;
        ...
        resultado.sucesso = resultado.erros === 0;
        resultado.mensagens.push(`Sincronização concluída: ${resultado.novos} novos, ${resultado.atualizados} atualizados, ${resultado.erros} erros`);
